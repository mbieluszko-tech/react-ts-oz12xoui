...
export function useData(config, session, currentOrg, onUnauthorized) {
  ...
  }, [currentOrg?.id, session?.token, loadData]);
...
export function useMyOrgs(config, session, onUnauthorized) {
  ...
  const load = useCallback(async () => {
    if (!config?.url || !config?.key || !session?.token || !session?.email) return;
    ...
    try {
      ...
      const members = await client.get(
        ...
        `&select=organization_id,role,status,organizations!inner(id,name,slug,type,description,color,logo_emoji,active,archived_at,archived_by)`
      );
      ...
      // pokazujemy wszystkie organizacje użytkownika, także zarchiwizowane
      // nie filtrujemy po organizations.active ani po archived_at
      const orgs = members
        .map(m => m.organizations)
        .filter(Boolean);

      const uniqueOrgs = Array.from(new Map(orgs.map(o => [o.id, o])).values());
      setMyOrgs(uniqueOrgs);
...
export function useTokenRefresh(config, session, onRefreshed) {
  ...
  }, [config, session?.token, session?.expires_at, session?.refresh_token, session?.email, onRefreshed]);
}
