  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filtered.map(m => {
        ...
        return [m.name, m.email||"", m.phone||"", sec?.name||"", ROLE_LABELS[m.role]||m.role, m.joined_at?fmt(m.joined_at):"", m.rodo_accepted?"Tak":"Nie", m.terms_accepted?"Tak":"Nie"];
      })
    ];
    ...
    <>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{data.members.filter(m=>m.status==="active").length} aktywnych · {data.members.filter(m=>m.status==="pending").length} oczekujących</div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          {isManager && <button className="btn btn-secondary" onClick={exportCSV}><Icon name="list" size={15}/> Eksport CSV</button>}
          {isAdmin   && <button className="btn btn-primary"   onClick={onAddMember}><Icon name="plus" size={15}/> Dodaj członka</button>}
        </div>
      </div>
      <div className="content">
        <div style={{ display:"flex", gap:12, marginBottom:20, alignItems:"center", flexWrap:"wrap" }}>
          <input className="form-input" style={{ maxWidth:280 }} placeholder="Szukaj po nazwisku lub emailu..." value={search} onChange={e=>setSearch(e.target.value)}/>
        ...
        {!filtered.length && <div className="empty-state"><Icon name="users" size={40}/><p style={{ marginTop:12 }}>Brak wyników</p></div>}
                  ...
                  return (
                    <div key={m.id} className="member-card" onClick={()=>onSelectMember(m)}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div className="member-card-name" style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{m.name}</div>
                        {m.role!=="member" ? <div className={"role-badge role-"+m.role}>{ROLE_LABELS[m.role]}</div> : <div className="member-card-role">{sec.name}</div>}
            ...
            <div>
              ...
              <div className="members-grid">
                {noSec.map(m=>{
                  ...
                  return (
                    <div key={m.id} className="member-card" onClick={()=>onSelectMember(m)}>
                      <div className="member-av" style={{ background:"var(--bg4)", color:"var(--text3)" }}>{initials}</div>
                      <div>
