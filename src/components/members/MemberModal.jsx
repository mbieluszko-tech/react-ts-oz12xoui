...
export function MemberModal({ member, sections, appointments, getReplies, onClose, onUpdate, isAdmin }) {
  ...
  // Statystyki — useMemo żeby nie liczyć przy każdym renderze
  const stats = useMemo(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaving(false);
      // Błąd jest obsługiwany przez handleUpdateMember w App.jsx (toast)
      // Nie zamykamy editing — użytkownik widzi że operacja nie powiodła się
    }
  };
      ...
      <div className="modal modal-wide" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-label={`Profil: ${member.name}`}>
        ...
        <div className="modal-header">
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            <div className="member-av" style={{ width:48, height:48, fontSize:18, background:`linear-gradient(135deg,${sec?.color||"var(--gold)"}88,${sec?.color||"var(--gold)"}33)`, color:sec?.color||"var(--gold)" }}>{initials}</div>
            <div>
              ...
              <div>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <select id="em-role" className="form-input" value={form.role} onChange={e=>setF("role",e.target.value)}>
