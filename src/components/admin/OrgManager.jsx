...
export function OrgManager({ organizations, onCreateOrg, onUpdateOrg, onClose }) {
      ...
      <div className="modal modal-wide" style={{ maxWidth:720 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding:"0 28px", borderBottom:"1px solid var(--border)", display:"flex", gap:4 }}>
          {[["list","Istniejące grupy"],["create","+ Dodaj nową grupę"]].map(([k,v]) => (
            <button key={k} onClick={()=>{ setTab(k); setEditOrg(null); }}
...
function OrgList({ organizations, onEdit }) {
  return (
    <div>
...
function OrgForm({ editOrg, onSave, onCancel }) {
  ...
  // Zmień domyślne sekcje gdy zmieni się typ
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      await onSave({ ...form, sections: editOrg ? undefined : sections });
    } finally { setSaving(false); }
  };

  const valid = form.name.trim().length > 1 && form.slug.length > 1;

  return (
    <div>
      <div style={{ fontSize:13, fontWeight:600, color:"var(--text3)", textTransform:"uppercase", letterSpacing:".07em", marginBottom:20 }}>
        {editOrg ? `Edycja: ${editOrg.name}` : "Nowa grupa"}
      </div>

      {/* Podgląd */}
      <div style={{ display:"flex", alignItems:"center", gap:14, padding:"16px 20px", background:"var(--bg3)", borderRadius:12, marginBottom:24, border:`1px solid ${form.color}44` }}>
        <div style={{ width:52, height:52, borderRadius:14, background:`${form.color}22`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>
          {form.logo_emoji}
        </div>
        <div>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:18, fontWeight:600, color:form.color }}>
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input className="form-input" placeholder="np. Chór ACK UG" value={form.name} onChange={e => handleNameChange(e.target.value)}/>
        </div>
        <div className="form-group">
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input className="form-input" placeholder="np. chor-ack" value={form.slug}
            onChange={e => setF("slug", e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,""))}/>
        </div>
        <div className="form-group">
          <label className="form-label">Emoji / logo</label>
          <input className="form-input" value={form.logo_emoji}
            onChange={e => setF("logo_emoji", e.target.value)} style={{ fontSize:22 }}/>
        </div>
      </div>
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <input type="color" value={form.color} onChange={e => setF("color", e.target.value)}
              style={{ width:44, height:36, borderRadius:8, border:"1px solid var(--border)", background:"var(--bg3)", cursor:"pointer", padding:2 }}/>
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="Krótki opis grupy" value={form.description}
      ...
      {/* Status */}
      <div className="form-group">
        <div className="checkbox-row" onClick={() => setF("active", !form.active)}
            ...
            {sections.map((sec, idx) => (
              <div key={idx} style={{ display:"flex", gap:8, alignItems:"center" }}>
                <input type="color" value={sec.color}
                  ...
                  style={{ width:36, height:36, borderRadius:8, border:"1px solid var(--border)", background:"var(--bg3)", cursor:"pointer", padding:2, flexShrink:0 }}/>
                <input className="form-input" placeholder="Nazwa sekcji" value={sec.name}
                  onChange={e => handleSectionChange(idx, "name", e.target.value)}/>
                <button className="btn btn-danger btn-sm" onClick={() => removeSection(idx)}
